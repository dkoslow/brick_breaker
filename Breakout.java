...
/** Runs the Breakout program. */
	public void run() {
		...
		if(brickCounter ==0) {
			...
			add(endGame, getWidth() / 2, (getHeight() + label.getAscent()) / 2)
	...
	private void createBricks() {
		for ( int x = 0; x < NBRICKS_PER_ROW; x++ ) {
			for (int y = 0; y < NBRICK_ROWS; y++ ) {
				brick = new GRect( getWidth() / 2 - ( NBRICKS_PER_ROW / 2 *BRICK_WIDTH ) + BRICK_WIDTH * x, BRICK_Y_OFFSET + ( y * BRICK_HEIGHT ), BRICK_WIDTH, BRICK_HEIGHT );
